...
/**
 * Composite dedup key. Two cart rows are the same iff (sku, sourceInvoiceId)
 * match. The same SKU from two different source invoices MUST yield two
 * distinct rows so per-invoice attribution survives through to the SS order
 * submission (FR-016 / spec 002-printavo-order-notification).
 */
function sameRow(a, b) {
  return a.sku === b.sku && a.sourceInvoiceId === b.sourceInvoiceId;
}
...
/**
 * Remove a row identified by (sku, sourceInvoiceId).
 */
export function removeItem(cart, sku, sourceInvoiceId) {
  return touch(cart, cart.items.filter((i) => !(i.sku === sku && i.sourceInvoiceId === sourceInvoiceId)));
}

/**
 * Set qty for the row identified by (sku, sourceInvoiceId). qty ≤ 0 is
 * equivalent to removeItem.
 */
export function setQty(cart, sku, sourceInvoiceId, qty) {
  const next = clampQty(qty);
  if (next <= 0) return removeItem(cart, sku, sourceInvoiceId);
  return touch(
    cart,
    cart.items.map((i) => (i.sku === sku && i.sourceInvoiceId === sourceInvoiceId ? { ...i, qty: next } : i)),
  );
}
