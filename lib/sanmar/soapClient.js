...
/**
 * SanMar whitelists a fixed outbound IP address per integration — Vercel's
 * serverless functions have no static egress IP by default, so all SanMar
 * calls are routed through Fixie (usefixie.com), a managed static-IP HTTP
 * proxy, when FIXIE_URL is configured. Node's global `fetch()` is backed by
 * undici, which accepts a proxy via the `dispatcher` option — no separate
 * HTTP client library needed. When FIXIE_URL is unset (e.g. local dev),
 * requests go out directly, unchanged from before.
 */
