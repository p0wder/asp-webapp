 ...
 * @property {string} identifier — SS SKU
 * @property {number} qty
...
/**
 * For each line item, decide its source: SS Activewear if at least one cart
 * row from this checkout matched it; otherwise we treat it as Sanmar.
 * Unresolved-lookup state is not produced by this function — it's a
 * pre-cart-add state that doesn't survive into the cart submission.
 */
function sourceOf(lineItem, cartItems) {
  const hit = cartItems.some((c) => c.sourceLineItemId === lineItem.id);
  return hit ? 'ss-activewear' : 'sanmar';
}
...
export function classifyInvoices({ cartItems, invoices, currentStatusByInvoiceId }) {
  return invoices.map((invoice) => {
    ...
    const allSSActivewear = lineItems.every((li) => li.source === 'ss-activewear');
    const allFullyOrdered = lineItems.every((li) => li.cartQty >= li.invoiceQty);

    if (allSSActivewear && allFullyOrdered) {
      return {
        sourceInvoiceId: invoice.id,
        sourceInvoiceVisualId: invoice.visualId,
    ...
    const reasons = [];
    if (!allSSActivewear) reasons.push('contains Sanmar lines');
    if (!allFullyOrdered) {
      const missing = lineItems.filter((li) => li.cartQty < li.invoiceQty).length;
      reasons.push(`${missing} line(s) under-quantity`);
    }
    return {
      sourceInvoiceId: invoice.id,
      sourceInvoiceVisualId: invoice.visualId,
...
export function computePartialItemsSummary({ invoice, priorAttributionRecords }) {
  ...
  const lineItems = (invoice.lineItems || []).map((li) => {
    const alreadyOrdered = li.source === 'ss-activewear' ? (alreadyOrderedByLineItem.get(li.sourceLineItemId) || 0) : 0;
    return {
