...
/**
 * Geocode a location string to lat/lng + city/state.
 * Returns null if the location cannot be resolved.
 * @param {string} query
 * @returns {Promise<{ lat: number, lng: number, city: string|null, stateCode: string|null } | null>}
 */
export async function geocodeLocation(query) {
  ...
  try {
    ...
    console.error('[geocode] geocodeLocation failed:', err.message);
    return null;
  }
  ...
  const stateCode = addr.state ? (US_STATE_ABBR[addr.state] ?? addr.state) : null;
