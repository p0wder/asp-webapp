...
export function generateQuoteApprovalToken(invoiceId, secret) {
  if (!invoiceId || !secret) throw new Error('invoiceId and secret are required');
  return createHmac('sha256', secret).update(`quote-approval:${String(invoiceId)}`).digest('hex');
}

export function verifyQuoteApprovalToken(invoiceId, token, secret) {
  if (!invoiceId || !token || !secret) return false;
  try {
    const expected = generateQuoteApprovalToken(invoiceId, secret);
    if (token.length !== expected.length) return false;
    return timingSafeEqual(Buffer.from(token, 'hex'), Buffer.from(expected, 'hex'));
  } catch {
    return false;
