 ...
 * (e.g. Sport-Tek items SS doesn't carry) are classified as 'ss-miss' — this does
 * NOT mean "route to Sanmar" by itself; the caller (lib/vendorCatalogLookup.js)
 * is responsible for trying a real SanMar lookup for any 'ss-miss' item before
 * giving up. Items with null/empty styleNumber are also classified 'ss-miss'.
 *
 ...
 * @returns {Promise<Array<{ lineItemId: string, state: 'matched'|'ss-miss'|'failed', variants?: Array, error?: string }>>}
 */
export async function lookupVariantsForLineItems(items) {
  ...
  for (const item of items) {
    ...
    if (!style) {
      // No style number — can't be an SS item, defer to SanMar
      results.set(item.lineItemId, { lineItemId: item.lineItemId, state: 'ss-miss', variants: [] });
      continue;
    }
  ...
  await Promise.all(
    Array.from(byStyle.entries()).map(async ([rawStyle, groupItems]) => {
      try {
        ...
        if (!styleMeta) {
          // Style not found in SS catalog — defer to SanMar
          console.log(`[ssActivewear] Style "${rawStyle}" not found in SS catalog — ss-miss`);
        ...
        for (const item of groupItems) {
          ...
          if (filtered.length === 0) {
            // Style exists in SS but this specific color isn't available — defer to SanMar
            console.log(`[ssActivewear] Style "${style}" color "${item.color}" not found in SS — ss-miss`);
            results.set(item.lineItemId, { lineItemId: item.lineItemId, state: 'ss-miss', variants: [] });
          } else {
