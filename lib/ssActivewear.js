 ...
 *
 * Groups items by `styleNumber`, hits `/v2/products/?style={name}` once per group, then
 * filters variants by `color` (case-insensitive) where the request specifies one. Items
 * with null/empty `styleNumber` are classified `sanmar` without an upstream call.
 *
...
export async function lookupVariantsForLineItems(items) {
  ...
  await Promise.all(
    Array.from(byStyle.entries()).map(async ([style, groupItems]) => {
      try {
        // Prefer styleid lookup via STYLE_ID_MAP (more reliable than ?style= bare number).
        // The ?style= endpoint requires "BrandName StyleNumber" format and rejects bare
        // style numbers like "6210". Fall back to brand+style string if not in the map.
        let variants;
        const styleMapEntry = STYLE_ID_MAP[style];
        if (styleMapEntry) {
          variants = await fetchSSProduct(String(styleMapEntry.styleID), { filterBy: 'styleid' });
        } else {
          // Unknown style — try the ?style= endpoint with the bare name as a last resort
          variants = await fetchSSProduct(style, { filterBy: 'style' });
        }
