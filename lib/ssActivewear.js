 ...
 * in `lib/ssOrderingSwitch.js`. Read, catalog, status and reconciliation calls
 * in this module are deliberately NOT gated — they must stay available while
 * ordering is disabled so open orders can still be reconciled (TG-001-02 AC2).
 */

import { assertSSOrderingEnabled } from './ssOrderingSwitch.js';
 ...
 *
 * IMPORTANT: testOrder is now hardcoded to false — orders placed through
 * this function are LIVE and will be charged/shipped for real.
 *
 ...
 *
 * shippingMethod is hardcoded to 54 (free freight) — SS defaults to 1 (paid)
 * when this field is omitted, which is why API orders were being charged
 * shipping that the same order gets for free on ssactivewear.com. Confirmed
 * with SS Activewear support.
 *
...
export async function createSSOrder({ shippingAddress, lines, poNumber, comments, paymentProfileId } = {}) {
  ...
  // MUST stay the first statement in this function. Evaluating it before
  // credentials are read or a request body is built is what guarantees that a
  // disabled switch produces zero calls to the supplier.
  ...
  const body = {
    // ⚠️  LIVE — real orders now place for real. Flipped intentionally to
    //     investigate a shipping-cost discrepancy vs. ordering on the website.
    testOrder: false,
    ...
    // Without this, SS defaults to shippingMethod 1 (paid freight) and never
    // applies our account's free-freight terms, unlike orders placed on the
    // website. Confirmed with SS support: 54 = free freight.
    shippingMethod: 54,
