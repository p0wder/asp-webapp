  ...
  const results = await Promise.all(
    invoiceIds.map((id) =>
      gql(
        ...
        { id },
      ).then((data) => data.invoice).catch(() => null),
    ),
  );
  return results
    .filter(Boolean)
...
/**
 * Fetch an invoice together with the fields needed to derive its balance
 * (TG-001-04).
 *
 * Separate from `getInvoiceById` on purpose: this is the authoritative read
 * behind a real-money decision, and it must fail loudly rather than return a
 * partially-populated object that the amount derivation would then guess at.
 *
 * ── The two-step query ───────────────────────────────────────────────────
 * `amountOutstanding` / `amountPaid` are the fields Printavo exposes for an
 * invoice balance, but this project has no way to introspect the live schema
 * from CI. If that query is rejected the request is retried with `total`
 * alone, and the caller is told which basis it got via `balanceFieldsPresent`.
 * `derivePayableCents` then falls back to the total, which is a *higher*
 * amount than the outstanding balance — the safe direction to be wrong in, and
 * it is recorded rather than hidden.
 *
 * @param {string} id Printavo node ID.
 * @returns {Promise<object|null>} Invoice fields, or null when not found.
 */
export async function getInvoicePaymentSnapshot(id) {
  if (!id) return null;
  ...
  try {
    ...
    return { ...data.invoice, balanceFieldsPresent: true };
  } catch (err) {
    console.warn(
      '[printavo] Balance fields unavailable, falling back to invoice total:',
      err?.message || err,
    );
  }

  const data = await gql(
    `query GetInvoiceTotal($id: ID!) {
      invoice(id: $id) {
        ${baseFields}
      }
    }`,
  ...
  if (!data.invoice) return null;
  return { ...data.invoice, balanceFieldsPresent: false };
}

/**
 * Record a payment against a Printavo invoice.
 *
 * Moved out of `app/api/stripe-webhook/route.js`, where the mutation was
 * written inline in the handler — the only external mutation in the repo that
 * was not behind a `lib/` client, and therefore the only one outside the
 * audit-logging convention (Principles III and V, variance V9).
 *
 * @param {object} params
 ...
 */
export async function recordInvoicePayment({ invoiceId, amountCents, paymentMethod = 'Credit Card' }) {
  if (!invoiceId) throw new Error('recordInvoicePayment requires an invoiceId');
  if (!Number.isInteger(amountCents) || amountCents <= 0) {
    throw new Error('recordInvoicePayment requires a positive integer amountCents');
  }

  const data = await gql(
    `mutation RecordPayment($invoiceId: ID!, $amount: Float!, $paymentMethod: String!) {
      paymentCreate(parentId: $invoiceId, input: { amount: $amount, paymentMethod: $paymentMethod }) {
        id
        amount
      }
    }`,
    { invoiceId, amount: amountCents / 100, paymentMethod },
  );

  return data.paymentCreate;
}
...
export async function getInvoicesByCustomerEmail(email) {
  ...
  const fields = `id visualId nickname total createdAt customerDueAt status { id name color } contact { id fullName email }`;

  const [quotesData, invoicesData] = await Promise.all([
    gql(`query QuotesByEmail($q: String) { quotes(first: 25, query: $q) { nodes { ${fields} } } }`, { q: email }),
    gql(`query InvoicesByEmail($q: String) { invoices(first: 25, query: $q) { nodes { ${fields} } } }`, { q: email }),
  ]);

  const quotes = (quotesData.quotes?.nodes || []).filter(n => n.contact?.email?.toLowerCase() === lower);
  const invoices = (invoicesData.invoices?.nodes || []).filter(n => n.contact?.email?.toLowerCase() === lower);

  // Merge and sort by createdAt descending, deduplicate by id
  const seen = new Set();
...
/**
 * List customers from Printavo, newest first.
 * Uses cursor-based pagination; `page` is 1-based and maps to 25-item pages.
 * Returns { customers: [{ id, companyName, email, phone, createdAt }], hasNextPage }
 */
export async function listCustomers(page = 1) {
  ...
  // Slice for offset-style pagination (Printavo GQL uses cursors; this is good enough for the dashboard)
  const sliced = offset > 0 ? nodes.slice(offset) : nodes;

  return {
    customers: sliced.map((c) => ({
      id: c.id,
      companyName: c.companyName || '',
      name: c.primaryContact?.fullName || c.companyName || '',
