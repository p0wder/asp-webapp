...
export const READY_TO_ORDER_STATUS_ID = '256605';
export const GOODS_IN_TRANSIT_STATUS_ID = '292756';
...
export async function getInvoicesByCustomerEmail(email) {
  if (!email) return [];
  const data = await gql(
    `query InvoicesByEmail($query: String) {
      invoices(first: 50, query: $query) {
        nodes {
          id
          visualId
          nickname
