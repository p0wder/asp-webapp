 ...
 * Used by the order status and proof APIs to look up invoice details, and by
 * `/api/create-payment-session` to derive what may be charged.
 *
 * `amountOutstanding` is the server-owned balance and is the ceiling on any
 * Stripe charge (TG-001-04). It is selected here — not computed by a caller
 * from `total` — because a caller subtracting payments it does not know about
 * is how a partly-paid invoice gets billed twice. Money fields are Floats in
 * dollars; convert with `dollarsToCents` from `lib/paymentAmount.js`.
 *
 * @param {string} id — Printavo node ID
 * @returns {Promise<{ id, visualId, nickname, total, subtotal, amountPaid, amountOutstanding, paidInFull, customerDueAt, status: { id, name }, contact: { id, fullName, email } }|null>}
 */
